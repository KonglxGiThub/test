...
public class FindSekerById extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String text = request.getParameter("searchInput").trim();
		dao = new SekDao();
		try {
			lists = dao.findAllId(now,text);
			if(lists.size()==0){
				lists = dao.findAllId(1,text);
			...
			page.setNow(now);
			request.setAttribute("servlet", "FindSekerById?a="+1);
