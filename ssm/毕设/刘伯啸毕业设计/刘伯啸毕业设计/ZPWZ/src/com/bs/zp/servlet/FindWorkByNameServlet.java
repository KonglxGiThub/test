...
public class FindWorkByNameServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		dao = new JobDao();
		try {
			lists = dao.findAllJob(now,txtKeyWord);
			if(lists.size()==0){
				lists = dao.findAllJob(1);
			...
			page = pc.countPage(daoP.findAllJob());
			page.setNow(now);
			request.setAttribute("servlet", "FindAllJob?a="+1);
