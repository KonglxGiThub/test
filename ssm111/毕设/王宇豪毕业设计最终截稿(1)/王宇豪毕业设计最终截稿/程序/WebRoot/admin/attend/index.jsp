...
function top(){
   	form3.action="<%=basePath%><%=dir %>/attend/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/attend/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/attend/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/attend/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/attend/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/attend/index.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    <%//eid,type,money,ftime,remark,addtime
			...
			int cou = eb.getMessageCountAtt();//得到信息总数			        
			...
			List pagelist1 = eb.getMessageAtt(Integer.parseInt(page1));
...
%>  
	...
	<TABLE width="100%" border=0 align="right" cellPadding=0 cellSpacing=0>
	 ...
	 <a href="<%=basePath %>admin/attend/add.jsp?method=addAtt"><font color=red>增加产品安装信息</font></a>	
