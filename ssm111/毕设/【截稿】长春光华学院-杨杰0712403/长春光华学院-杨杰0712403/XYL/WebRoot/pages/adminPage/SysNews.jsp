  		...
  		<script type="text/javascript" charset="utf-8">
			...
			$(function() {
				datagrid4 = $('#datagrid4').datagrid({
					...
					toolbar : [ {
						...
						iconCls : 'icon-reload',
						handler : function() {
							$('#datagrid4').datagrid({ url:'AdminViewNewsServlet' }) ;
					...
					columns : [ [
					{
						...
						styler: function(index,row){
							if(row.class_id==1){
								return 'background-color:#E8FFC4;';
							}
						}
					},{
						title : '操作',
			...
			function EditEmployee() {
				...
				if (row) {
					...
					url = 'AdminEditNewsServlet?id=' + row.id;
				}
			}
            ...
            if (row){  
                $.messager.confirm('删除警告', ss.join('<br/>'),function(r){  
                    if (r){  
                        $.post('AdminDelNewsServlet',{id:ro},function(result){  
