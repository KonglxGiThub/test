  ...
  <body >
  		...
  		<script type="text/javascript" charset="utf-8">
			...
			$(function() {
				datagrid1 = $('#datagrid1').datagrid({
					...
					toolbar : [ {
						...
						iconCls : 'icon-reload',
						handler : function() {
							$('#datagrid1').datagrid({ url:'ViewAdminUserServlet' }) ;
			...
			function EditEmployee() {
				...
				if (row) {
					$('#dlg1').dialog('open').dialog('setTitle', '管理员账号管理  >>  编辑');
					$('#fm1').form('load', row);
					...
					url = 'AdminEditUserServlet?id=' + row.id;
				}
			}
            ...
            if (row){  
                $.messager.confirm('删除警告', ss.join('<br/>'),function(r){  
                    if (r){  
                        $.post('AdminDelUserServlet',{id:ro},function(result){  
			...
			function CheckEmployeeDetails() {
				var row = $('#datagrid1').datagrid('getSelected');
				if (row) {
					$('#dlg1').dialog('open').dialog('setTitle', '管理员账号管理  >>  查看详情');
					$('#fm1').form('load', row);
			...
			function NewEmployee() {
				$('#dlg1').dialog('open').dialog('setTitle', '管理员账号管理  >>  新增');
				$('#fm1').form('clear');
				$('#dlg1').find('input').attr('disabled',false);
				$('#dlg1').find('#button1').show();
				$('#button2').html("<span class='l-btn-left'><span class='l-btn-text icon-cancel l-btn-icon-left'>取消</span></span>");
				url = 'AdminNewUserServlet';
			}
		</script>
	<div class="easyui-layout" data-options="fit:true" style="position:absolute;">
		...
		<div id="dlg1" class="easyui-dialog"
			style="width: 600px; height: 250px; padding:20px" data-options="title: '管理员账号管理  >>  新增',closed: true,
						closable:false, 
						cache: false,  
						modal: true,
						resizable:true"
			...
			<form id="fm1" method="post" novalidate >
				<table  class="table1" id="user_table">
					<tr>
						...
						<th>职位：</th>
						<td> 
							<input id="cc" name="role" class="easyui-combobox" data-options="
							required:true,
							editable:false,
							valueField: 'value',
							textField: 'label',
